...
function normalizeAddon(input) {
  ...
  return { id: String(id), addon_id: firstAvailable(row, ['addon_id', 'id'], null), label: name, price: money(firstAvailable(row, ['price', 'display_price', 'amount'], 0)), unit: firstAvailable(row, ['unit'], 'one-time'), addon_category: firstAvailable(row, ['addon_category', 'category'], null), notes: firstAvailable(row, ['notes', 'description'], ''), freezone: firstAvailable(row, ['freezone', 'freezone_name'], null), is_active: bool(firstAvailable(row, ['is_active', 'active'], true), true), source: 'supabase', raw: row };
}
function uniqueBy(items, getKey) {
  const seen = new Set();
  return items.filter((item) => { const key = getKey(item); if (!key || seen.has(key)) return false; seen.add(key); return true; });
}
async function selectFirstWorkingTable(candidates) {
  let lastError = null;
  for (const { table, query, normalize } of candidates) {
    try { const rows = await supabaseRest.select(table, query); return { table, rows: (rows || []).map(normalize).filter((row) => row.is_active !== false) }; }
...
function newUuid() {
  return typeof window !== 'undefined' && window.crypto?.randomUUID ? window.crypto.randomUUID() : 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (c) => { const r = (Math.random() * 16) | 0; return (c === 'x' ? r : (r & 0x3) | 0x8).toString(16); });
}

function authToken() {
  const token = localStorage.getItem('ssu_token');
  if (!token) throw new Error('Please sign in before starting an application.');
  return token;
}
...
function buildOrderPayload(draft, totalAed, user) {
  ...
  const couponCode = draft.coupon_code || localStorage.getItem('ssu_scratch_coupon_code') || null;
  ...
  const noteParts = [`Ref: ${reference}`, business.activity ? `Activity: ${business.activity}` : null, (business.company_names || []).filter(Boolean).length ? `Company names: ${(business.company_names || []).filter(Boolean).join(', ')}` : null, draft.office_type ? `Office: ${draft.office_type}` : null].filter(Boolean);
  return {
    token,
    id: newUuid(), reference, user_id: user?.id || null, coupon_code: couponCode,
    customer_name: contact.name || null, customer_email: contact.email || null, customer_phone: contact.phone ? `${contact.phone_code || ''} ${contact.phone}`.trim() : null,
    freezone: draft.zone_name || draft.zone_slug || 'Free Zone', package_id: draft.package_id || null, package_name: draft.package_name || draft.zone_name || null,
    duration_years: money(draft.duration_years, 1), visa_count: money(draft.visa_count, 0), shareholder_count: money(business.shareholders, 1),
    base_price: basePrice, addons_total: addonsTotal, discount_total: 0, final_total: finalTotal, currency: 'AED', status: 'draft', notes: noteParts.join(' | ') || null,
  };
}

async function recalculateOrder(orderId, token) {
  return supabaseRest.rpc('recalculate_checkout_order', { p_order_id: orderId }, token);
}
...
export async function createCheckoutOrder(draft, totalAed, user) {
  ...
  const { reference, token, ...orderRow } = orderPayload;
  ...
  const recalculated = await recalculateOrder(orderId, token);
  const authoritative = Array.isArray(recalculated) ? recalculated[0] : recalculated;
  return { id: orderId, reference, ...orderRow, ...(authoritative ? { final_total: authoritative.grand_total, discount_total: authoritative.discount_total, addons_total: authoritative.addon_total, base_price: authoritative.base_price } : {}) };
}
...
export async function markBankTransferSubmitted(order, bankProof) {
  ...
  return supabaseRest.update('checkout_orders', { status: 'payment_review', notes: noteSuffix }, query, token);
}
