...
async function selectFirstWorkingTable(candidates) { let lastError = null; for (const { table, query, normalize } of candidates) { try { const rows = await supabaseRest.select(table, query); return { table, rows: (rows || []).map(normalize).filter((row) => row.is_active !== false) }; } catch (error) { lastError = error; } } throw lastError || new Error('Supabase pricing table not available'); }
...
function buildOrderPayload(draft, totalAed, user) {
  const reference = buildOrderReference(); const contact = draft.contact || {}; const business = draft.business || {}; const token = authToken(); const couponCode = draft.coupon_code || localStorage.getItem('ssu_scratch_coupon_code') || null;
  const addonsTotal = (draft.addons || []).reduce((sum, a) => sum + money(a.price, 0), 0); const finalTotal = money(totalAed, 0); const basePrice = Math.max(finalTotal - addonsTotal, 0);
...
export async function markBankTransferSubmitted(order, bankProof) {
  ...
  return supabaseRest.rpc('submit_bank_transfer_proof', { p_order_id: order.id, p_note: noteSuffix }, token);
}
