...
const NAV = [
  ...
  { label: 'Corporate Services', children: [
        ...
        <div className="flex items-center justify-between h-[72px] gap-6">
          <Link to="/" className="flex items-center gap-2 group shrink-0">
            ...
            <div className="leading-tight">
              ...
              <div className="text-[10px] uppercase tracking-[0.2em] text-slate-500 flex items-center gap-1"><ShieldCheck className="h-3 w-3 brand-emerald" /> UAE Business Setup Platform</div>
            </div>
          </Link>
          ...
          <nav ref={navRef} className="hidden lg:flex items-center gap-6 flex-1 justify-center">
            {NAV.map((n) => n.children ? (
              <div key={n.label} className="relative">
                <button type="button" onClick={() => setOpenMenu((menu) => (menu === n.label ? '' : n.label))} className="text-sm font-medium text-slate-700 hover:text-slate-900 flex items-center gap-1 link-underline">
                ...
                {openMenu === n.label && (
                  <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 w-60 bg-white rounded-2xl shadow-2xl border border-emerald-900/10 p-2 fade-up">
                    {n.children.map((c) => <Link key={c.href} to={c.href} className={`block px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${isActive(c.href) ? 'bg-emerald-50 brand-emerald' : 'text-slate-700 hover:bg-emerald-50 hover:brand-emerald'}`}>{c.label}</Link>)}
                  </div>
                )}
              </div>
            ) : (
              <Link key={n.href} to={n.href} className={`text-sm font-medium link-underline ${isActive(n.href) ? 'brand-emerald' : 'text-slate-700 hover:text-slate-900'}`}>{n.label}</Link>
            ))}
          </nav>
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            {user ? (
              <div className="relative" ref={acctRef}>
                <button type="button" onClick={() => setAcct((a) => !a)} className="flex items-center gap-2 px-3 py-2 rounded-full border border-slate-200 hover:border-emerald-700/30 bg-white transition-colors">
                ...
                {acct && (
                  <div className="absolute top-full mt-2 right-0 w-56 bg-white rounded-2xl shadow-2xl border border-emerald-900/10 p-2 fade-up">
                    ...
                    <button type="button" onClick={() => { logout(); navigate('/'); }} className="w-full text-left flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-700 hover:bg-red-50 hover:text-red-600"><LogOut className="h-4 w-4" /> Logout</button>
                  </div>
                )}
              </div>
          ...
          <button type="button" className="lg:hidden p-2 -mr-2" onClick={() => setOpen(!open)} aria-label="Menu">
          ...
          <div className="px-5 py-4 flex flex-col gap-1">
            {NAV.map((n) => n.children ? (
              <div key={n.label} className="py-1">
                ...
                {n.children.map((c) => <Link key={c.href} to={c.href} className="block py-2 px-2 font-medium text-slate-800">{c.label}</Link>)}
              </div>
            ...
            {user ? <><Link to="/dashboard" className="py-2 px-2 font-medium text-slate-800">Dashboard</Link><button type="button" onClick={() => { logout(); navigate('/'); }} className="text-left py-2 px-2 font-medium text-red-600">Logout</button></> : <Link to="/login" className="py-2 px-2 font-medium text-slate-800">Client Login</Link>}
