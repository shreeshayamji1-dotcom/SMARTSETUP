...
export default function AdminPanel() {
  ...
  const loadRoles = useCallback(async () => {
    ...
    try {
      ...
      setRoleRows([...(profiles || []), ...(admins || [])].filter((r) => ADMIN_ROLES.includes(r.role)));
    } catch { setRoleRows([]); }
  }, []);
  ...
  <section className="bg-white py-10"><div className="max-w-7xl mx-auto px-5 lg:px-8 space-y-6">
    ...
    {tab==='roles'&&<div className="card-elevated rounded-3xl p-7"><div className="text-xs uppercase tracking-[0.22em] font-semibold text-slate-500">Roles</div><div className="mt-2 text-lg font-semibold">Admin / Manager / Staff / Reviewer</div><p className="mt-3 text-slate-600">UI role checks do not replace database RLS.</p><div className="mt-5 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left text-slate-500 border-b"><th className="py-3">Name</th><th>Email</th><th>Role</th><th>Active</th></tr></thead><tbody>{roleRows.map(r=><tr key={r.id||r.user_id||r.email} className="border-b border-slate-100"><td className="py-3 font-medium">{r.full_name||r.name||'-'}</td><td>{r.email||'-'}</td><td>{r.role}</td><td>{r.is_active===false?'No':'Yes'}</td></tr>)}{!roleRows.length&&<tr><td colSpan="4" className="py-8 text-center text-slate-500">No admin role rows loaded.</td></tr>}</tbody></table></div></div>}
